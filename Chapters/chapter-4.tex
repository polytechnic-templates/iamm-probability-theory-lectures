...
\]
\end{definition}
\subsection{Свойства характеристичекой функции случайного вектора}
\begin{enumerate}
...
\begin{enumerate}
	\item Сходимость с вероятностью 1. \\
	...
	Говорят, что $\{ \xi_n \}$ стремится по вероятности к $\xi$, если $\forall \varepsilon > 0$:
	\[
		\lim\limits_{n \to \infty} \Prob \{ |\xi_n - \xi | > \varepsilon \} = 0
	\]
	\item Сходимость в среднем квадратическом. \\
...
При большом числе испытаний относительная частота успеха стремится по вероятности к вероятности появления успеха. Число испытаний растёт, значит, отночистельная частота реже отличается от вероятности.
\begin{theorem}
